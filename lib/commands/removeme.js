...
module.exports = {
    ...
    code: async (chan, chanUID, sender, senderUID, message) => {
        if (message[1] === 'live') {
            ...
            const res = await utils.query(`SELECT peopleLive FROM live WHERE channelUID = ${mysql.escape(chanUID)}`)
        ...
        if (message[1] === 'offline') {
            ...
            const res = await utils.query(`SELECT peopleOffline FROM live WHERE channelUID = ${mysql.escape(chanUID)}`)
