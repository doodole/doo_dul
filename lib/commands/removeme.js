...
module.exports = {
    ...
    code: async (chan, chanUID, sender, senderUID, message) => {
        if (message[1] === 'live') {
            ...
            con.query(
                `UPDATE live 
                SET peopleLive = REPLACE(peopleLive, ${mysql.escape(sender)}, '') 
                WHERE channelUID = ${mysql.escape(chanUID)}`
            )
            const [res] = await con.promise().query(
                `SELECT peopleLive FROM live 
                WHERE channelUID = ${mysql.escape(chanUID)}`
            )
            const cleantext = res[0].peopleLive.replace(/\s+/g, " ").trim()
            con.query(
                `UPDATE live 
                SET peopleLive = ${mysql.escape(cleantext)}
                WHERE channelUID = ${mysql.escape(chanUID)}`
            )
        ...
        if (message[1] === 'offline') {
            ...
            con.query(
                `UPDATE live
                SET peopleOffline = REPLACE(peopleOffline, ${mysql.escape(sender)}, '')
                WHERE channelUID = ${mysql.escape(chanUID)}`
            )
            const [res] = await con.promise().query(
                `SELECT peopleOffline FROM live 
                WHERE channelUID = ${mysql.escape(chanUID)}`
            )
            const cleantext = res[0].peopleOffline.replace(/\s+/g, " ").trim()
            con.query(
                `UPDATE live 
                SET peopleOffline = ${mysql.escape(cleantext)} 
                WHERE channelUID = ${mysql.escape(chanUID)}`
            )
