    ...
    code: async (chan, chanUID, sender, senderUID, message) => {
        ...
        if (['rl', 'randomline'].includes(message[0])) {
            if (message[1] === undefined) {
                const sql = `SELECT t1.sender, t1.date, t1.message FROM logs AS t1
                JOIN (SELECT senderUID FROM chatters WHERE chanUID = ${mysql.escape(chanUID)} ORDER BY RAND() LIMIT 1) AS t2
                ON t1.senderUID=t2.senderUID`
                const [results] = await con.promise().query(sql)
                if (results.length === 0) {
                    return { 'say': `This channel has no logs!` }
                }
                const randRow = results[Math.floor(Math.random() * results.length)]
                return determineTimeElapsed(randRow.date, randRow.sender, randRow.message)
            } else {
                if (message[1].toLowerCase() == "titlechange_bot") { return }
                const sql = `SELECT date, sender, message FROM logs WHERE chanUID = ${mysql.escape(chanUID)} AND sender = ${mysql.escape(message[1])} ORDER BY RAND() LIMIT 1`
                const [results] = await con.promise().query(sql)
                if (!results.length) {
            ...
            if (message[1] === undefined) {
                const sql = `SELECT date, sender, message FROM logs WHERE chanUID = ${mysql.escape(chanUID)} AND senderUID = ${mysql.escape(senderUID)} ORDER BY RAND() LIMIT 1`
                const [results] = await con.promise().query(sql)
                if (results.length === 0) {
