...
module.exports = {
    ...
    code: async (chan, chanUID, sender, senderUID, message) => {
        ...
        if (time.match(/^[0-9]+$/) === null && ![`ban`, `perma`, `permaban`].includes(time)) {
            ...
            return { 'say': `${sender}, Timeout must be at most 1209600 (seconds) or "perma" for a permaban` };
        } else {
            const [channelBans] = await con.promise().query(
            ...
            for (const i in channelBans) {
                if (channelBans[i].banphrase.toLowerCase() === ban.toLowerCase() && channelBans[i].timeout.toLowerCase() === time) {
                    ...
                    con.query(
                        `UPDATE banphrases 
                        SET timeout = ${mysql.escape(time)} 
                        WHERE channelUID = ${mysql.escape(chanUID)} AND banphrase = ${mysql.escape(ban)}`
                    )
                    return { 'say': `${sender}, Banphrase updated!` }
                }
            }
            con.query(
                `INSERT INTO banphrases 
                VALUES (${mysql.escape(chanUID)}, ${mysql.escape(chan)}, ${mysql.escape(ban)}, ${mysql.escape(time)})`
            )
