...
module.exports = {
    ...
    code: async (chan, chanUID, sender, senderUID, message) => {
        if (message[1] === 'follow') {
            const channels = await utils.query(`SELECT * FROM follownotifications WHERE channel = ${mysql.escape(chan)}`)
            ...
            con.query(`DELETE FROM follownotifications WHERE channel = ${mysql.escape(chan)}`)
        ...
        if (message[1] === 'live') {
            ...
            con.query(`DELETE FROM live WHERE channel = ${mysql.escape(chan)}`)
