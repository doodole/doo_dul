...
module.exports = {
    ...
    code: async (chan, chanUID, sender, senderUID, message) => {
        if (message[1] === 'follow') {
            const [channels] = await con.promise().query(`SELECT channel FROM follownotifications WHERE channel = ${mysql.escape(chan)}`)
            ...
            const [channels] = await con.promise().query(`SELECT channel FROM live WHERE channel = ${mysql.escape(chan)}`)
            if (channels.length !== 0) {
