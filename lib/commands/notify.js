    ...
    code: async (chan, chanUID, sender, senderUID, message) => {
        if (message[1] === 'follow') {
            ...
            con.query(
                ...
                VALUES (${mysql.escape(chan)}, ${mysql.escape(chanUID)}, 'has followed PogChamp', ${mysql.escape(data.data[0].id)})`
            )
            return { 'say': 'You will now be notified when a user follows you!' }
        } else if (message[1] === 'live') {
            const [channels] = await con.promise().query(
                `SELECT channel FROM live 
                WHERE channel = ${mysql.escape(chan)}`
            )
            if (channels.length !== 0) {
                ...
                con.query(
                    ...
                    WHERE channelUID = ${mysql.escape(chanUID)}`
                )
            ...
            con.query(
                ...
                VALUES (${mysql.escape(chan)}, ${mysql.escape(chanUID)}, 'false', ${mysql.escape(chan + ' has gone live PogChamp')}, ${mysql.escape(chan + ' has gone offline FeelsBadMan')}, ${mysql.escape(onlinedata.data[0].id)}, ${mysql.escape(offlinedata.data[0].id)}, 'yes')`
            )
