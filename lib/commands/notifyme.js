...
module.exports = {
    ...
    code: async (chan, chanUID, sender, senderUID, message) => {
        if (message[1] === 'live') {
            const channels = await utils.query(`SELECT * FROM live WHERE channel = ${mysql.escape(chan)}`)
            if (channels.length === 0 || channels[0].notifications === 'no') {
                return {'say': `This channel is not taking live notifications`};
            }
            else if (channels[0].peopleLive === null) {
                con.query(`UPDATE live SET peopleLive = ${mysql.escape(sender)} WHERE channelUID = ${mysql.escape(chanUID)}`)
            ...
            con.query(`UPDATE live SET peopleLive = CONCAT(peopleLive, ${mysql.escape(' ' + sender)}) WHERE channelUID = ${mysql.escape(chanUID)}`)
            return {'say': 'You will now be notified when the streamer goes live!'}
        }
        if (message[1] === 'offline') {
            ...
            else if (channels[0].peopleOffline === null) {
                con.query(`UPDATE live SET peopleOffline = ${mysql.escape(sender)} WHERE channelUID = ${mysql.escape(chanUID)}`)
            ...
            con.query(`UPDATE live SET peopleOffline = CONCAT(peopleOffline, ${mysql.escape(' ' + sender)}) WHERE channelUID = ${mysql.escape(chanUID)}`)
            return {'say': 'You will now be notified when the streamer goes offline!'}
        }
    },
    userCooldown : 10000,
