    ...
    code: async (chan, chanUID, sender, senderUID, message) => {
        if (message[1] === 'live') {
            ...
            if (channels.length === 0 || channels[0].notifications === 'no') {
                ...
                con.query(
                    `UPDATE live SET peopleLive = ${mysql.escape(sender)} 
            ...
            con.query(
                `UPDATE live 
                SET peopleLive = CONCAT(peopleLive, ${mysql.escape(' ' + sender)}) 
                WHERE channelUID = ${mysql.escape(chanUID)}`
            )
            return { 'say': 'You will now be notified when the streamer goes live!' }
        }
        if (message[1] === 'offline') {
            ...
            if (channels.length === 0 || channels[0].notifications === 'no') {
                ...
                con.query(
                    `UPDATE live 
                    SET peopleOffline = ${mysql.escape(sender)} 
                    WHERE channelUID = ${mysql.escape(chanUID)}`
                )
            ...
            con.query(
                `UPDATE live 
                SET peopleOffline = CONCAT(peopleOffline, ${mysql.escape(' ' + sender)}) 
                WHERE channelUID = ${mysql.escape(chanUID)}`
            )
            return { 'say': 'You will now be notified when the streamer goes offline!' }
        }
